...
type ObjectFilterInput<T> = Partial<
   Record<LogicalOperator, Maybe<FilterInput<T>>[]>
> & {
...
export const FilterInputType = <T extends object>(
   ...
   const className = withReferences
      ? `${target.name}FilterInput`
      : `${target.name}BasicFilterInput`
   ...
   Field(() => [withoutReferenceFilterType], { nullable: true })(
   ...
   for (const [key, options] of Object.entries(properties).filter(
      ...
      if (options.name != key)
         Expose({ name: options.name, toClassOnly: true })(
            Placeholder.prototype,
            key,
         )
   }
